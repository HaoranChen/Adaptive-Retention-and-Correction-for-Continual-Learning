...
class Learner(BaseLearner):
    ...
    def incremental_train(self, data_manager):
        ...
        self.test_loader = DataLoader(test_dataset, batch_size=self.args["arc_batch_size"], shuffle=False, drop_last=False, num_workers=num_workers)
        ...
        if self.args["adaptive_retention"]:
            ...
            if self.args["train_prompt"]:
                optimizer = optim.Adam([
                    {'params': self._network.fc.parameters(), 'lr': self.args["adjust_head_lr"]}, 
                    {'params': self._network.prompt.parameters(), 'lr': self.args["adjust_prompt_lr"]}    # Group 2 with different settings
            ], weight_decay=self.weight_decay)
            else:
                optimizer = optim.Adam([
                    {'params': self._network.backbone.fc.parameters(), 'lr': self.args["adjust_head_lr"]}, 
                ], weight_decay=self.weight_decay)
