...
class Learner(BaseLearner):
    ...
    def after_task(self):
        ...
        # self.save_checkpoint(self.log_path+'/'+self.model_prefix+'_seed{}_tune'.format(self.seed), head_only=self.fix_bcb)
    ...
    def incremental_train(self, data_manager):
        ...
        self.test_loader = DataLoader(test_dataset, batch_size=self.args["arc_batch_size"], shuffle=False, num_workers=num_workers)
    ...
    def _eval_cnn(self, loader):
        ...
        if self.args["adaptive_retention"]:
            ...
            for p in self._network.backbone.e_prompt.parameters():
                p.requires_grad = True

            self._network.backbone.g_prompt.requires_grad = True
            ...
            if self.args["train_prompt"]:
                ...
                {'params': self._network.backbone.e_prompt.parameters(), 'lr': self.args["adjust_prompt_lr"]},
                {'params': self._network.backbone.g_prompt, 'lr': self.args["adjust_prompt_lr"]}
            ], weight_decay=self.weight_decay)
            else:
                optimizer = optim.Adam([
                    {'params': self._network.backbone.head.parameters(), 'lr': self.args["adjust_head_lr"]}, 
