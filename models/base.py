    ...
    def _eval_cnn(self, loader):
        ...
        if self.args["adaptive_retention"]:
            for name, param in self._network.named_parameters():
                if 'fc' in name and 'block' not in name:
            ...
            optimizer = optim.SGD(self._network.fc.parameters(), lr=self.args["adjust_lr"], momentum=0.9)
        
        for _, (_, batch_inputs, batch_targets) in enumerate(loader):
            inputs = batch_inputs.to(self._device)  
            targets = batch_targets.to(self._device)

            outputs = self._network(inputs)['logits']  # Forward pass on the whole batch
            predicts = torch.topk(outputs, k=self.topk, dim=1, largest=True, sorted=True)[1]  # Top-k predictions
            ...
            # Append predictions and targets for the batch
